    ...
    if (appError) {
      return (
        <div className="p-4">
          ...
          <button onClick={() => { setAppError(null); if (!apiKey) { handleClearApiKey() } else { setCurrentView('input') } }}
        ...
        return (
            selectedTask && <TaskDetailModal
              ...
              onUpdate={(updatedTask) => {
                setTasksWithHistory(prevTasks => prevTasks.map(t => t.id === updatedTask.id ? updatedTask : t));
                setSelectedTask(updatedTask);
              }}
              onDelete={handleRemoveTask}
              onOpenSlideEditor={handleCustomReportGenerated}
              onSubStepUpdate={(subSteps) => {
                if (selectedTask) {
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
              ...
              onSubStepAdd={(title, description) => {
                if (selectedTask) {
                  const newSubStep: SubStep = {
                    id: generateUniqueId('substep'),
                    text: title,
                    notes: description,
                    position: getDefaultSubStepPosition(selectedTask.extendedDetails?.subSteps?.length || 0)
                  };
              ...
              onSubStepDelete={(subStepId) => {
                if (selectedTask) {
                  const updatedSubSteps = selectedTask.extendedDetails?.subSteps?.filter(ss => ss.id !== subStepId) || [];
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
                      ...selectedTask.extendedDetails!,
              ...
              onSubStepStatusChange={(subStepId, status) => {
                if (selectedTask) {
                  const updatedSubSteps = selectedTask.extendedDetails?.subSteps?.map(ss => 
                    ss.id === subStepId ? { ...ss, status } : ss
                  ) || [];
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
              ...
              onAttachmentAdd={(attachment) => {
                if (selectedTask) {
                  ...
                  const updatedAttachments = [...(selectedTask.extendedDetails?.attachments || []), newAttachment];
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
                      ...selectedTask.extendedDetails!,
              ...
              onAttachmentDelete={(attachmentId) => {
                if (selectedTask) {
                  const updatedAttachments = selectedTask.extendedDetails?.attachments?.filter(a => a.id !== attachmentId) || [];
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
                      ...selectedTask.extendedDetails!,
              ...
              onDecisionAdd={(decision) => {
                if (selectedTask) {
                  const newDecision: Decision = {
                    ...decision,
                    id: generateUniqueId('decision')
                  };
                  const updatedDecisions = [...(selectedTask.extendedDetails?.decisions || []), newDecision];
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
                      ...selectedTask.extendedDetails!,
              ...
              onDecisionUpdate={(decisionId, updates) => {
                if (selectedTask) {
                  const updatedDecisions = selectedTask.extendedDetails?.decisions?.map(d => 
              ...
              onDecisionDelete={(decisionId) => {
                if (selectedTask) {
                  const updatedDecisions = selectedTask.extendedDetails?.decisions?.filter(d => d.id !== decisionId) || [];
                  const updatedTask = {
                    ...selectedTask,
                    extendedDetails: {
                      ...selectedTask.extendedDetails!,
